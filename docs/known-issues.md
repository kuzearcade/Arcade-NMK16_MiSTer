...
### NMK-32 · Savestates: what a state does not carry
- **Cores:** all four · **Severity:** limitation · **Status:** open (by design, 2026-09-18)
- **Ref:** `docs/hw-bringup.md` "Savestates"; `rtl/savestate/`
- A state is the complete CPU/MCU/RAM/register image (see the core-side
  map in each core's `ss_*` port comment) plus a shadow of every FM
  register write, replayed into the chip after a load. Not restored:
  - the OKIM6295s' internal playback state (a sample that was sounding at
    the save is silent after a load; the next trigger plays normally);
  - the FM chips' envelopes and phases -- the register file is replayed,
    key-on registers are replayed as key-off, so a note that was held at
    the save is cut and the tune resumes at the game's next key-on;
  - the raster position: the machine is parked and resumed at a VBlank
    edge, so a load lands at the start of a frame (interrupts pending in
    the V-PROM state machine at that moment are taken after the resume,
    the same way after a save and after a load, which is what the
    round-trip test relies on);
  - the video's two-stage sprite buffer: it is refilled by the sprite DMA,
    which keeps running while the machine is parked, so a load is held
    two extra frames before the release to let both stages take the
    restored table (otherwise the first two frames show the old sprites);
  - `tharrierb` (id 55): its MC68705R3 (`rtl/m68705`) has no park
    mechanism, so the Savestates page is hidden there (`status_menumask`
    bit 2).
- A save takes ~2 frames (park at VBlank, transfer, release at the next
  VBlank), a load ~4 (two more for the sprite buffers, above). While it runs the core's Pause (OSD, hiscore, cheats)
  is masked, since the CPUs must execute a few instructions to park. If
  a CPU cannot park (held in reset by the NMK004 watchdog, halted by the
  protection MCU during its boot handshake) the operation aborts after
  ~100 ms with "Savestate failed" and the game continues untouched.
- With a Vert orientation (the DDR3 rotation framebuffer) the engine
  shares the DDRAM port with `screen_rotate` and yields to every one of
