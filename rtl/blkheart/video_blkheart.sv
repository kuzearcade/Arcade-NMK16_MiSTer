// blkheart-family (Family B/macross) video pipeline — the third Tier 2
// NMK004-board game to get a real video pipeline, after
// rtl/mustang/video_mustang.sv and rtl/bioship/video_bioship.sv (those
// modules' own headers cover the shared tile-decode/palette/sprite-FSM
// architecture in full; this header only covers what's genuinely
// different for blkheart). blkheart is structurally much closer to
// mustang than bioship was: same single-BG-layer-plus-TX layout
// (`VIDEO_START_MEMBER(nmk16_state,macross)`, `screen_update_macross`,
// `gfx_macross` — blkheart reuses these exact same MAME helper
// functions, not just similar ones), so this module is a close
// derivative of video_mustang.sv with two real, targeted differences:
//
//   - **BG tile-code bank extension.** blkheart's own "bgtile" graphics
//     ROM is 0x100000 bytes (8192 16x16 tiles) — double the 4096-tile
//     range a bare 12-bit VRAM tile code addresses. `common_get_bg_tile_
//     info<0,1>` (nmk16_v.cpp) ORs in `m_bgbank<<12` as a 13th code bit,
//     with `m_bgbank` set by `tilebank_w` (byte @0x080019 in
//     blkheart_core.sv's own memory map). mustang's own bgtile ROM is
//     exactly 0x80000 (4096 tiles, 12 bits exactly), so
//     video_mustang.sv never needed this. Here, `bg_code = {bg_bank[0],
//     bgvram_data[11:0]}` feeds `group16_pixel()`'s own `base` argument
//     directly (only bit 0 of the 8-bit `bg_bank` register matters,
//     since 8192 tiles = 13 bits = 12 VRAM bits + exactly 1 bank bit).
//   - **Real BG Y-scroll**, not just X. blkheart's own scroll register
//     is the generic `scroll_w<0>` template (nmk16.cpp/nmk16_v.cpp),
//     which — unlike mustang's own X-only, mode-selector-byte
//     `mustang_scroll_w` — gives BG a genuine independent 16-bit Y
//     register alongside X. `bg_yscroll` is a new input here, applied
//     with the same wraparound-modulo technique bg_xscroll already
//     uses (and the same technique rtl/bioship/video_bioship.sv's own
//     BG0/BG1 scroll already established for a second game).
//
// Everything else — tile-decode functions, palette-decode, sprite
// double-buffer delay, sprite draw FSM, TX-over-sprite-over-BG
// compositing order, palette color bases (TX=0x200, BG=0x000,
// sprites=0x100 — blkheart uses the *exact same* `gfx_macross` GFXDECODE
// as mustang, unlike bioship's own different `gfx_bioship` bases), and
// screen geometry (SCREEN_W/H, VIDEOSHIFT=92, MAX_SPRITE_CLOCK=134656) —
// is unchanged from video_mustang.sv. See that module's own header for
// their full derivation, not repeated here.
module video_blkheart #(
	...
	// register/RAM read ports into blkheart_core's storage (dual-tap
	// reads, blkheart_core owns the arrays — see mustang_core.sv's own
	// header for why)
	...
	// ------------------------------------------------------------------
	reg [7:0] fgtile_rom  [0:131071];  // 3.bin, 8x8x4bpp packed_msb, 32B/tile
	reg [7:0] bgtile_rom  [0:1048575]; // 90068-5.bin, 16x16 col_2x2_group, 128B/tile — 8192 tiles (bank-extended, see header)
	reg [7:0] sprites_rom [0:1048575]; // 90068-8.bin, word_swap-extracted (see tools/mkgfxrom.py), 128B/16x16-unit
	...
	// through both halves uniformly (same layout as mustang's/bioship's
	...
	// logical px), independent X and Y scroll (see header — mustang's
	// own is X-only). Pure per-pixel combinational function, same
	// rationale as video_mustang.sv's own tilemap path.
	// ------------------------------------------------------------------
	...
	// is unchanged for blkheart (same 4-bit colour mask, same sprite
