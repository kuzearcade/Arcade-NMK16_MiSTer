// hachamfb-family (Family B/macross) video pipeline — the eighth Tier 2
...
// rtl/acrobatm/video_acrobatm.sv, rtl/tdragon/video_tdragon.sv, and
// rtl/vandyke/video_vandyke.sv (those modules' own headers cover the
// shared tile-decode/palette/sprite-FSM architecture in full).
// hachamfb's own video family, ROM sizes, and sprite-ROM loading
// convention are all identical to blkheart's own — same
// `VIDEO_START_MEMBER(macross)`, `screen_update_macross`, `gfx_macross`
// (TX=0x200, BG=0x000, sprites=0x100), same 0x100000-byte bgtile ROM
// (8192 tiles, genuinely needing the `tilebank_w` bank-extension this
// time — unlike vandyke's own inert case), same 0x100000-byte
// `ROM_LOAD16_WORD_SWAP` sprite ROM — every one of these confirmed
// directly against `ROM_START(hachamfb)`, not assumed from the
// "hachamf" name family (see hachamfb_core.sv's own header for the
// up-front hardware-family verification this port did before writing
// any RTL). This module is therefore functionally IDENTICAL to
// video_blkheart.sv (copied with only header/module-name changes). See
// that module's own header (and video_mustang.sv's, for the base
// architecture) for the full derivation of everything below, not
// re-derived here since nothing about it is actually different for
// hachamfb.
module video_hachamfb #(
	...
	// register/RAM read ports into hachamfb_core's storage (dual-tap
	// reads, hachamfb_core owns the arrays — see mustang_core.sv's own
	// header for why)
	...
	// ------------------------------------------------------------------
	reg [7:0] fgtile_rom  [0:131071];  // 5.95, 8x8x4bpp packed_msb, 32B/tile
	reg [7:0] bgtile_rom  [0:1048575]; // 91076-4.101, 16x16 col_2x2_group, 128B/tile — 8192 tiles (bank-extended, see header)
	reg [7:0] sprites_rom [0:1048575]; // 91076-8.57, word_swap-extracted (see tools/mkgfxrom.py), 128B/16x16-unit
	...
	// is unchanged for hachamfb (same 4-bit colour mask, same sprite
