// gunnail video pipeline — Family D (TLCS-90 protection MCU, NMK-215/
// TMP90840 variant), macross's own sibling. Reuses video_macross.sv's
// own base architecture (tile/palette decode, sprite double-buffer/draw
// FSM, dual-NMK214 descramble wiring) verbatim, with two genuine
// differences confirmed directly from the reference:
//
//   1. **Real per-scanline raster X+Y scroll** — `docs/PLAN.md`'s own
//      "hardest video item identified" for this hardware family.
//      Confirmed directly from `nmk16_v.cpp`'s own `bg_update()`
//      (lines 372-397): for each screen row `y1`, `yscroll =
//      scrollramy[0] + scrollramy[y1]`, and the row's own X-scroll is
//      `scrollram[0] + scrollram[16+y1]` (the `+16` offset is the
//      reference's own literal starting value for its `i` counter —
//      not derived/explained further in the source, kept as-is here
//      rather than guessed away). Since this project's own rendering
//      model is demand-driven per-pixel (`rd_x`/`rd_y` inputs, not a
//      real scanline-by-scanline hardware draw pass — see every prior
//      port's own `video_*.sv`), implementing this is actually simpler
//      than real hardware would need: `bg_line_y`/`bg_line_x` below
//      just read `scrollramy[rd_y]`/`scrollram[16+rd_y]` directly per
//      pixel instead of using a single frame-constant scroll register.
//      Not independently pixel-verified against a real capture this
//      session (flagged honestly, not claimed pixel-perfect) — the
//      `+16` offset and the exact row-vs-tilemap-row relationship
//      MAME's own tilemap-cache machinery implies are the most likely
//      source of any remaining error if the rendered result isn't
//      exactly right.
//   2. **Wider TX tilemap** — `VIDEO_START_MEMBER(nmk16_state,gunnail)`
//      calls `VIDEO_START_CALL_MEMBER(macross2)` first (`nmk16_v.cpp:
//      164`), which creates a 64x32-tile TX tilemap
//      (`TILEMAP_SCAN_COLS,8,8,64,32`) — double macross's own 32x32 —
//      matching `gunnail_core.sv`'s own doubled TX VRAM (2048 words).
//      The reference's own comment on this ("leftmost 64 pixels have
//      to be retrieved from the other side of the tilemap") describes
//      why `set_scrolldx(28+64,28+64)` is needed, not a separate
//      addressing mechanism — the existing modular-wraparound formula
//      (`% <logical width>`) already produces this once the logical
//      width doubles to 512, no new special-case logic needed.
//
//   BG tile code is **13 bits, not macross's own 14** — gunnail's own
//   bgtile ROM is 0x100000 bytes (1MB, half macross's own 2MB): at 128
//   bytes/16x16-tile, that's 8192 tiles, matching blkheart/tdragon's
//   own convention (`bg_bank[0]` only, not macross's own `bg_bank[1:0]`).
//   Sprites stay the same 2MB ROM as macross's own, unchanged wiring.
//
// Everything else (palette decode, TX pixel decode, sprite double-
// buffer/draw FSM, NMK214 descramble instances and their bitswap
// tables — shared, game-independent constants in the reference) is
// video_macross.sv's own, unchanged — not re-derived here.
module video_gunnail #(
	...
	// register/RAM read ports into gunnail_core's storage (dual-tap
	// reads, gunnail_core owns the arrays)
	...
	// Per-scanline raster scroll — see header. scrollram_0/scrollramy_0
	// are the fixed index-0 base values; *_row_addr/*_row are the
	// dynamic per-row taps this module drives (scrollram_row_addr =
	// 16+rd_y, scrollramy_row_addr = rd_y).
	...
	localparam integer VIDEOSHIFT = 92; // set_scrolldx(28+64,28+64), see header
	...
	// ------------------------------------------------------------------
	reg [7:0] fgtile_rom  [0:131071];  // 1.u21, 8x8x4bpp packed_msb, 32B/tile
	reg [7:0] bgtile_rom  [0:1048575]; // 92077-4.u19, 16x16 col_2x2_group, 128B/tile — 8192 tiles (13-bit code, see header)
	reg [7:0] sprites_rom [0:2097151]; // 92077-7.u134, word_swap-extracted, 128B/16x16-unit
	...
	// ------------------------------------------------------------------
	// BG tile-fetch address, computed as a plain wire so it can drive
	// the nmk214 BG instance's own `addr` input continuously — see
	// header. gfx_8x8x4_col_2x2_group_packed_msb layout: 128 bytes/
	// 16x16 unit, left half (col 0-7) at +0, right half (col 8-15) at
	// +64, same as every prior port's own derivation.
	// ------------------------------------------------------------------
	wire [19:0] bg_byte_addr;
	...
	nmk214 #(
		...
		// Shared, game-independent table (same array macross's own
		// nmk214_bg uses) — bit 20 never sets for gunnail's own 1MB
		// ROM (20-bit address space), harmless.
		...
		.addr({1'b0, bg_byte_addr}), .din(16'h0), .dout_word(),
	...
	// instance, unchanged from video_macross.sv's own (same 2MB ROM).
	// ------------------------------------------------------------------
	...
	nmk214 #(
		...
		.ADDR_WIDTH(21),
		// nmk214_sprites_address_bitswap, nmk16.cpp:5683 — shared table.
	...
	// logical px) — same dimensions as macross's own (macross2's own
	// VIDEO_START creates the SAME 256x32 BG tilemap size, only TX
	// widens — see header). Per-scanline X/Y scroll (bg_xscroll_row/
	// bg_yscroll_row above) replaces macross's own frame-constant
	// bg_xscroll/bg_yscroll.
	// ------------------------------------------------------------------
	...
	// common_get_bg_tile_info<0,1>: (code&0xfff)|(m_bgbank<<12) — 13-bit
	// code for gunnail's own 8192-tile ROM, see header (bg_bank[0] only,
	// matching blkheart/tdragon's own smaller-ROM convention, not
	// macross's own bg_bank[1:0]).
	wire [12:0] bg_code = {bg_bank[0], bgvram_data[11:0]};
	wire [3:0]  bg_half_col = bg_px;
	assign bg_byte_addr = {bg_code, 7'd0} + (bg_half_col >= 4'd8 ? 20'd64 : 20'd0) + {14'd0, bg_py, 2'd0} + {18'd0, bg_half_col[2:1]};
	...
	// macross's own 32x32/256x256 — see header), no scroll beyond the
	...
	// video_macross.sv's own (see video_mustang.sv's own header
	// derivation) — ping-ponging 2048-entry buffers.
	// ------------------------------------------------------------------
	...
	// is unchanged for gunnail (same 4-bit colour mask, same sprite
	...
	// ------------------------------------------------------------------
	// Sprite draw FSM — unchanged from video_macross.sv's own.
	// ------------------------------------------------------------------
	localparam
		S_RESET_CLR0 = 0,
		S_RESET_CLR1 = 1,
