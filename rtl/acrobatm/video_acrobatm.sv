// acrobatm-family (Family B/macross) video pipeline — the fifth Tier 2
// NMK004-board game to get a real video pipeline, after
// rtl/mustang/video_mustang.sv, rtl/bioship/video_bioship.sv,
// rtl/blkheart/video_blkheart.sv, and rtl/strahl/video_strahl.sv (those
...
// different for acrobatm). acrobatm's own video family is *identical*
// to blkheart's own (`VIDEO_START_MEMBER(nmk16_state,macross)`,
// `screen_update_macross`, `gfx_macross` — same single-BG-layer-plus-TX
// layout, same BG tile-code bank extension via `tilebank_w`, same
// palette color bases: TX=0x200, BG=0x000, sprites=0x100) — this module
// is a close derivative of video_blkheart.sv, differing only in the
// sprite ROM's own size.
//
//   - **BG tile-code bank extension** — identical mechanism to
//     blkheart's own: acrobatm's own "bgtile" ROM is 0x100000 bytes
//     (8192 16x16 tiles, same size as blkheart's own), so
//     `common_get_bg_tile_info<0,1>` ORs in `m_bgbank<<12` as a 13th
//     code bit, `bg_bank` set by `acrobatm_core.sv`'s own `tilebank_w`.
//   - Sprite ROM is bigger than blkheart's own: 0x180000 bytes (two
//     plain-`ROM_LOAD`-concatenated files of *different* sizes,
//     0x100000 + 0x080000 — not same-size chips the way most of this
//     family's own sprite ROMs are) — needs a bigger flat array than
//     blkheart's own 0x100000, but the same `[20:0]` truncation width
//     already covers it (matches strahl's own sprite-ROM sizing
//     precedent, and nmk16spr.cpp's own sprite `code` field is a full
//     16-bit value regardless of ROM size, so no functional changes).
//
...
// MAX_SPRITE_CLOCK=134656 — acrobatm calls the exact same
// `set_screen_lowres`/`set_max_sprite_clock(384*263)` helpers every
// prior port's own machine config does) — is unchanged from
// video_blkheart.sv. See that module's own header (and
// video_mustang.sv's, for the base architecture) for their full
// derivation, not repeated here.
module video_acrobatm #(
	...
	// register/RAM read ports into acrobatm_core's storage (dual-tap
	// reads, acrobatm_core owns the arrays — see mustang_core.sv's own
	// header for why)
	...
	// ------------------------------------------------------------------
	reg [7:0] fgtile_rom  [0:131071];  // 3.ic79, region declared 0x20000, only first 0x10000 dumped — see header
	reg [7:0] bgtile_rom  [0:1048575]; // am-03.ic8, 16x16 col_2x2_group, 128B/tile
	reg [7:0] sprites_rom [0:1572863]; // am-01.ic42+am-02.ic29 concatenated (0x180000 total, see header)
	...
	// mustang's/blkheart's own gfx_macross usage.
	...
	// header derivation) — ping-ponging 2048-entry buffers. acrobatm
	// uses the family's own default m_sprdma_base=0x8000 bytes = 0x4000
	// words (no override — unlike strahl's own 0xF000), same as mustang/
	// bioship/blkheart.
	// ------------------------------------------------------------------
	...
	// is unchanged for acrobatm (same 4-bit colour mask, same sprite
	// GFXDECODE color base 0x100 as mustang's/blkheart's own).
	// ------------------------------------------------------------------
