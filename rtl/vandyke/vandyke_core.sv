// NMK16 MiSTerFPGA project — vandyke hardware family (Tier 2, NMK004
// boards) system-level integration.
//
// The seventh Tier 2 NMK004-board game, after mustang, bioship,
// blkheart, strahl, acrobatm, and tdragon (see those modules' own
// headers for the shared 68000/NMK004/jt03/jt6295/nmk_irq integration
// architecture this reuses wholesale). vandyke's own memory map is
// otherwise very close to mustang's own (same base addresses for IN0/
// IN1/DSW1/DSW2/NMK004 latches, same 0x88000 palette, same 0x90000 BG
// VRAM), but with three real, targeted differences below. Its CPU/
// NMK004 clock ratio (10MHz/8MHz) is identical to bioship's own, so
// this file's own clock-enable section reuses bioship_core.sv's own
// 40MHz architecture verbatim.
//
// Real, substantive differences from mustang_core.sv:
//   - **`vandyke_flipscreen_w` inverts the flip polarity** relative to
//     every other game's own `flipscreen_w` (nmk16_v.cpp's own comment:
//     "vandyke writes a 0 value when flip screen is enabled, contrary
//     to rest of games that write 1") — `flip_screen_reg` is captured
//     from `~oEdb[7:0]` here, not `oEdb[7:0]` directly. (This design
//     doesn't yet consume flip_screen_reg for actual rendering — same
//     "not yet wired to real HPS_IO" scope as every prior port — but
//     the *register value itself* is captured with the correct,
//     verified polarity for whenever that changes.)
//   - **`vandyke_scroll_w` packs BG X/Y scroll into a wider,
//     byte-shifted 4-word register** (nmk16_v.cpp): `m_vscroll[0..3]`
//     (word-granularity writes, all 4 recomputed together on every
//     write) with `scrollx = vscroll[0]*256 + (vscroll[1]>>8)`,
//     `scrolly = vscroll[2]*256 + (vscroll[3]>>8)` — a 24-bit-ish
//     computed value that MAME's own tilemap code then wraps modulo the
//     tilemap's real pixel width/height (4096/512) when rendering.
//     Rather than replicating the literal 24-bit multiply-and-shift
//     arithmetic, this design computes the *effective* post-modulo
//     value directly (only the low bits of that arithmetic ever
//     survive the wraparound): `bg_xscroll = {vscroll[0][3:0],
//     vscroll[1][15:8]}` (12 bits, exactly `(vscroll[0]*256+(vscroll[1]
//     >>8)) mod 4096`), `bg_yscroll = {vscroll[2][0], vscroll[3][15:8]}`
//     (9 bits, exactly `mod 512`) — verified algebraically, not
//     assumed: `(vscroll[0]*256) mod 4096 = (vscroll[0] mod 16)*256`,
//     i.e. only `vscroll[0]`'s own low 4 bits survive, shifted up by 8;
//     `vscroll[1]>>8` is already an 8-bit quantity, unaffected by the
//     mod. Same derivation for Y with mod 512/2.
//   - **A genuinely mysterious extra RAM block**, `0x094000-0x097FFF`
//     — the reference's own comment: `// what is this?`. Backed here as
//     a plain, side-effect-free 8192x16 RAM array (matching the
//     reference's own bare `.ram()`, no write handler, no read
//     consumer anywhere in the driver) so CPU accesses to it behave
//     correctly (read-back-what-was-written) without falling through to
//     this design's own "unmapped" default, exactly replicating the
//     reference's own "backing exists, meaning unknown" behavior.
//   - **TX VRAM lives at `0x09D000`, not `0x09C000`** — a different
//     offset from mustang's own, otherwise identical size (1024 x 16).
//   - `tilebank_w` exists in the memory map, but vandyke's own bgtile
//     ROM is exactly 0x080000 bytes (4096 tiles, a clean 12-bit code —
//     the same size as mustang's own, not blkheart's bigger 0x100000/
//     8192-tile one) — confirmed directly from `ROM_START(vandyke)`,
//     not assumed. The bank-extension wiring is included anyway (same
//     `{bg_bank[0], bgvram_data[11:0]}` pattern as blkheart's own) since
//     it's harmless when `bg_bank` never becomes non-zero, and matches
//     this session's own established practice of wiring a register that
//     exists in the memory map even when a specific game's own ROM
//     content never exercises it.
//   - The sprite ROM (`0x200000`) is loaded as **two independent
//     `ROM_LOAD16_BYTE` interleaved pairs concatenated together**
//     (`vdk-07.202`+`vdk-06.203` at region offset 0, `vdk-04.2-1`+
//     `vdk-05.3-1` at region offset 0x100000) — not a single interleave
//     across all four files. Extracted as two separate
//     `tools/mkgfxrom.py --mode interleave16` runs, concatenated with a
//     plain `cat` (both intermediate outputs are already one-byte-per-
//     line `$readmemh` text, so literal file concatenation produces the
//     correct combined byte stream directly — no new tooling needed).
//   - `fgtile` is genuinely smaller than every prior port's own —
//     0x10000 bytes (2048 tiles), fully populated (not the usual
//     oversized-region/half-populated pattern) — `video_vandyke.sv`'s
//     own `fgtile_pixel()` truncates to 16 bits (`[15:0]`), not the
//     usual `[16:0]`, to stay in-bounds for this smaller array.
//
// Memory map (see vandyke_map in mame/src/mame/nmk/nmk16.cpp):
...
//   080015         vandyke_flipscreen_w (W, byte) — INVERTED polarity, see above
//   080016-080017  nmk004_x0016_w (W, word) — standard NMI polarity
//   080019         tilebank_w (W, byte) — present, but inert for this game's own ROM size
//   08001F         NMK004 host-write latch (W, byte)
//   088000-0887FF  palette RAM, 1024 x 16
//   08C000-08C007  vandyke_scroll_w (BG X/Y scroll, packed 4-word register, see above)
//   090000-093FFF  BG tilemap VRAM, 8192 x 16
//   094000-097FFF  mysterious extra RAM, 8192 x 16 (plain, no known consumer)
//   09D000-09D7FF  tx tilemap VRAM, 1024 x 16
...
module vandyke_core #(
	...
	// (10MHz/8MHz 68000/NMK004 ratio, identical to vandyke's own).
	// ------------------------------------------------------------------
	...
	wire sel_unkram    = (byte_addr >= 24'h094000) && (byte_addr <= 24'h097FFF);
	wire sel_txvram    = (byte_addr >= 24'h09D000) && (byte_addr <= 24'h09D7FF);
	...
	// ------------------------------------------------------------------
	// Mysterious extra RAM (8192 x 16, 0x094000-0x097FFF) — plain,
	// side-effect-free, no known consumer (see header).
	// ------------------------------------------------------------------
	reg [15:0] unkram [0:8191];
	wire [12:0] unkram_addr = byte_addr[13:1];
	always @(posedge clk_sys) begin
		if (sel_unkram & cpu_write) begin
			if (~UDSn) unkram[unkram_addr][15:8] <= oEdb[15:8];
			if (~LDSn) unkram[unkram_addr][7:0]  <= oEdb[7:0];
		end
	end
	wire [15:0] unkram_dout = unkram[unkram_addr];
	...
	// ------------------------------------------------------------------
	// Dual-port video read taps — video_vandyke.sv's own live, per-pixel
	// reads into the same arrays above (vandyke_core.sv owns the
	...
	reg [7:0]  bgbank_reg;
	reg [15:0] vscroll_reg [0:3]; // vandyke_scroll_w's own m_vscroll[0..3]
	reg        nmi_level;

	// Effective post-modulo BG scroll — see header's algebraic derivation.
	// bg_xscroll[11:0] = (vscroll[0]*256+(vscroll[1]>>8)) mod 4096;
	// bg_yscroll[8:0]  = (vscroll[2]*256+(vscroll[3]>>8)) mod 512.
	wire [15:0] bg_xscroll = {4'd0, vscroll_reg[0][3:0], vscroll_reg[1][15:8]};
	wire [15:0] bg_yscroll = {7'd0, vscroll_reg[2][0], vscroll_reg[3][15:8]};
	...
	always @(posedge clk_sys) begin
		if (reset) begin
			...
			bgbank_reg      <= 8'h00;
			nmi_level       <= 1'b0;
			for (si = 0; si < 4; si = si + 1) vscroll_reg[si] <= 16'h0000;
		end else if (cpu_write) begin
			// vandyke_flipscreen_w: inverted polarity, see header.
			if (sel_flip & ~LDSn)     flip_screen_reg <= ~oEdb[7:0];
			if (sel_tilebank & ~LDSn) bgbank_reg       <= oEdb[7:0];
			if (sel_scroll) begin
				if (~UDSn) vscroll_reg[scroll_word_idx][15:8] <= oEdb[15:8];
				if (~LDSn) vscroll_reg[scroll_word_idx][7:0]  <= oEdb[7:0];
			end
	...
	// ------------------------------------------------------------------
	// OKIM6295 x2 — real jt6295. Reference uses XTAL(12'000'000)/3=4MHz
	// for both chips here (same 4MHz target as every prior port, yet
	// another different divisor source — doesn't matter, see
	// mustang_core.sv's own header). 1MHz from 40MHz clk_sys is an
	// exact /40, same as bioship_core.sv's own.
	// ------------------------------------------------------------------
	...
	always @(*) begin
		...
		else if (sel_unkram)  rdata = unkram_dout;
	...
	// interrupt generation (rtl/nmk_irq/nmk_irq.sv, driven by vandyke's
	// own dumped V-PROM — same CRC as mustang's own, confirmed shared
	// PROM content again).
	// ------------------------------------------------------------------
	...
	wire sprite_dma_trigger; // consumed by video_vandyke below
	nmk_irq #(
		.VTIMING_FILE(VTIMING_FILE)
	) irq_gen (
		.clk_sys(clk_sys),
		.table_sel(3'd0),
		.halfpop(1'b0),   // NMK-29: no half-populated V-PROM on this board.prom_we(1'b0), .prom_addr(12'd0), .prom_data(8'd0), // reference core: $readmemh only
		.reset(reset),
	...
	// rtl/vandyke/video_vandyke.sv's own header for the full derivation.
	// ------------------------------------------------------------------
	video_vandyke #(
