// vandyke-family (Family B/macross) video pipeline — the seventh Tier 2
// NMK004-board game to get a real video pipeline, after
// rtl/mustang/video_mustang.sv, rtl/bioship/video_bioship.sv,
// rtl/blkheart/video_blkheart.sv, rtl/strahl/video_strahl.sv,
// rtl/acrobatm/video_acrobatm.sv, and rtl/tdragon/video_tdragon.sv
// (those modules' own headers cover the shared tile-decode/palette/
// sprite-FSM architecture in full). vandyke's own video family is
// identical to mustang's/blkheart's own (`screen_update_macross`,
// `VIDEO_START_MEMBER(macross)`, `gfx_macross` — TX=0x200, BG=0x000,
// sprites=0x100), so this module is a close derivative differing only
// in ROM sizes:
//
//   - **`fgtile` is genuinely smaller** than every prior port's own —
//     0x10000 bytes (2048 tiles), fully populated (not the usual
//     oversized-region/half-populated pattern several prior games'
//     own fgtile ROMs have). `fgtile_pixel()` truncates to 16 bits
//     (`[15:0]`) here, not the usual `[16:0]`, to stay in-bounds for
//     this smaller array — confirmed directly from `ROM_START(vandyke)`,
//     not assumed to match the family's usual size.
//   - **BG tile-code bank extension is wired but inert** — vandyke's
//     own `tilebank_w` register exists in the memory map (same as
//     blkheart's own), but vandyke's own bgtile ROM is exactly
//     0x080000 bytes (4096 tiles, a clean 12-bit code — the same size
//     as mustang's own, not blkheart's bigger 0x100000/8192-tile one).
//     The `{bg_bank[0], bgvram_data[11:0]}` pattern is included anyway
//     (harmless when `bg_bank` never becomes non-zero) — see
//     vandyke_core.sv's own header for why this is still wired rather
//     than omitted.
//   - Sprite ROM (`0x200000`, two independent `ROM_LOAD16_BYTE`
//     interleaved pairs concatenated — see vandyke_core.sv's own
//     header) needs a wider truncation (`[20:0]`, same width every
//     prior port's own `group16_pixel()` already uses, comfortably
//     covering 0x200000 with room to spare — consistent with the
//     established "truncation wider than the exact minimum" convention).
//
// Everything else — tile-decode functions, palette-decode, sprite
// double-buffer delay, sprite draw FSM, TX-over-sprite-over-BG
// compositing order, and screen geometry (SCREEN_W/H, VIDEOSHIFT=92,
// MAX_SPRITE_CLOCK=134656) — is unchanged from video_mustang.sv. See
// that module's own header for the full derivation, not repeated here.
module video_vandyke #(
	...
	// register/RAM read ports into vandyke_core's storage (dual-tap
	// reads, vandyke_core owns the arrays — see mustang_core.sv's own
	// header for why)
	...
	// ------------------------------------------------------------------
	reg [7:0] fgtile_rom  [0:65535];   // vdk-3.222, 8x8x4bpp packed_msb, 32B/tile — smaller than usual, see header
	reg [7:0] bgtile_rom  [0:524287];  // vdk-01.13, 16x16 col_2x2_group, 128B/tile
	reg [7:0] sprites_rom [0:2097151]; // vdk-07/06+vdk-04/05 concatenated (0x200000 total, see header)
	...
	function automatic [3:0] group16_pixel(input integer base, input integer rom_sel, input integer row, input integer col);
		...
		begin
			...
			group16_pixel = rom_sel
				...
				: tile_nibble(bgtile_rom[byte_addr[18:0]],  col_local[0]);
		end
	endfunction
	...
	// header. Only bit 0 of bg_bank matters, and it's always 0 for
	// vandyke's own ROM content (wired anyway, see header).
	...
	// is unchanged for vandyke (same 4-bit colour mask, same sprite
