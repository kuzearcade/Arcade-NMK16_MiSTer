// raphero video pipeline — Tier 3's third port. A genuine hybrid of two
// already-built modules, confirmed directly from the reference rather
// than re-derived from first principles:
//
//   raphero() uses gfx_macross2 (nmk16.cpp:5569, same GFXDECODE as
//   macross2/tdragon2 — same 14-bit BG tile code, same 5-bit sprite
//   colour, same palette bases: BG=0x000/16, SPR=0x100/32, TX=0x300/16)
//   but MCFG_VIDEO_START_OVERRIDE(nmk16_state, gunnail) (nmk16.cpp:5571 —
//   the SAME video-start function `gunnail` itself uses: single BG layer
//   + real per-scanline raster X+Y scroll, NOT a new raphero-specific
//   video-start).
//
//   1. **Per-scanline scroll — video_gunnail.sv's own mechanism, reused
//      directly, not re-derived.** This project's rendering model is
//      demand-driven per-pixel (rd_x/rd_y inputs), not MAME's own
//      scanline-by-scanline draw loop — video_gunnail.sv already reduces
//      the reference's own bg_update() loop (nmk16_v.cpp:374-405,
//      `yscroll=scrollramy[0]+scrollramy[y1]`,
//      `set_scrollx((i+yscroll)&0x1ff, scrollram[0]+scrollram[i])` with
//      i=16+y1) to a simple direct read: bg_xscroll_row =
//      scrollram[0]+scrollram[16+rd_y], bg_yscroll_row =
//      scrollramy[0]+scrollramy[rd_y] — no MAME-tilemap-cache-specific
//      row-remapping needed at all in a per-pixel model. Reused verbatim
//      here (same scrollram_row_addr=16+rd_y / scrollramy_row_addr=rd_y
//      port shape as video_gunnail.sv's own).
//   2. **tilerambank BG-VRAM banking — video_macross2.sv's own mechanism,
//      re-sourced from a different register here.** raphero_scroll_w
//      (nmk16_v.cpp:295-311, distinct from macross2's own generic
//      scroll_w<Layer>) computes newbank=(gunnail_scrollram[0]>>12)&3 —
//      bits[13:12] of the 16-bit WORD at scrollram index 0 (NOT bits
//      [5:4] of an 8-bit byte register like macross2's own — this is a
//      genuinely different bit position, not assumed from family
//      resemblance). raphero's own bgvideoram is 0x10000 bytes
//      (nmk16.cpp:1129, same size as macross2's/tdragon2's own), giving
//      the same 2-bit tilerambank range (0-3). tilerambank_reg is derived
//      in raphero_core.sv itself (see that file's own header/body); this
//      module just consumes it the same way video_macross2.sv's own
//      bgvram_addr does.
//
// Everything else BG-tilemap-related (256x32 paged geometry, 14-bit tile
// code via bg_bank[1:0] — raphero's own bgtile ROM is 0x200000 bytes,
// identical size to macross2's own) is video_macross2.sv's own, unchanged.
// TX tilemap sizing (64x32) is video_gunnail.sv's own, unchanged (matches
// raphero's own doubled TX VRAM, 2048 words, same as gunnail's own).
//
// Genuinely new here: raphero's own sprites ROM is 0x600000 bytes (6MB —
// bigger than any prior port's own, verified directly against
// ROM_START(raphero), nmk16.cpp:8595-8598, three 0x200000 files rather
// than macross2's own two), needing a wider (23-bit, not 22-bit) sprite
// byte address throughout the sprite-fetch path.
module video_raphero #(
	...
	// register/RAM read ports into raphero_core's storage (dual-tap
	// reads, raphero_core owns the arrays)
	output [14:0] bgvram_addr,
	input  [15:0] bgvram_data,
	input  [1:0]  tilerambank, // see header — top 2 bits of bgvram_addr below
	...
	// 16+rd_y, scrollramy_row_addr = rd_y) — identical shape to
	// video_gunnail.sv's own port interface.
	...
	// Palette bases — see header. Same as video_macross2.sv's own (shared
	// gfx_macross2).
	...
	// ------------------------------------------------------------------
	// Per-scanline scroll taps — see header/port list. Identical shape to
	...
	// ------------------------------------------------------------------
	reg [7:0] fgtile_rom  [0:131071];  // rhp94099.1, 8x8x4bpp packed_msb, 32B/tile
	reg [7:0] bgtile_rom  [0:2097151]; // rhp94099.4, 16x16 col_2x2_group, 128B/tile — 16384 tiles (14-bit code)
	reg [7:0] sprites_rom [0:6291455]; // rhp94099.8+9+10, word_swap-extracted, 128B/16x16-unit — 0x600000 bytes (see header)
	...
	// ------------------------------------------------------------------
	// Sprite tile-fetch address — plain byte read, no descrambling. 23
	// bits wide (see header — 0x600000-byte ROM, wider than any prior
	// port's own).
	// ------------------------------------------------------------------
	...
	// logical px) — video_macross2.sv's own geometry, but PER-SCANLINE
	// X/Y scroll (bg_xscroll_row/bg_yscroll_row, not frame-constant —
	// see header).
	// ------------------------------------------------------------------
	...
	// plus tilerambank as the top 2 bits (see header — video_macross2.sv's
	// own mechanism, re-sourced).
	assign bgvram_addr = {tilerambank, bg_row[4], bg_col, bg_row[3:0]};

	// common_get_bg_tile_info<0,1>: (code&0xfff)|(m_bgbank<<12) — 14-bit
	// code for raphero's own 16384-tile ROM (bg_bank[1:0], same as
	// macross2's own).
	...
	// ------------------------------------------------------------------
	// TX tilemap: fixed 8x8 tiles, 64x32 (512x256 logical px — video_
	...
	// field, sprite GFXDECODE colour base 0x100 (same as video_macross2.sv's
	// own — shared gfx_macross2).
	// ------------------------------------------------------------------
	...
	// ------------------------------------------------------------------
	// Sprite draw FSM — unchanged from video_macross2.sv's own, except
	// the byte address is 23 bits wide (see header — raphero's own
	// sprite ROM is 0x600000 bytes/6MB, bigger than any prior port's own).
	// ------------------------------------------------------------------
	localparam
		S_RESET_CLR0 = 0,
		S_RESET_CLR1 = 1,
