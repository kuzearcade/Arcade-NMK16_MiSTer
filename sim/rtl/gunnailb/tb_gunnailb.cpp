// Testbench for rtl/gunnailb/gunnailb_core.sv — Tier 5's fifth system-level
// integration milestone, but architecturally different from the prior four
// (see that module's header): no Seibu Sound System, no IM0 vector
// arbitration to check — instead this checks whether the new Z80 + direct
// YM2203 + dual-soundlatch sound path, and the offline-descrambled
// bgtile/sprites GFX data (tools/decode_gunnailb_gfx.py), actually produce
// a correct render when driven by two real CPUs.
//
...
int main(int argc, char **argv) {
	...
	Vgunnailb_core top{&contextp};

	FILE *z80_trace = std::fopen("gunnailb_z80.trace", "w");
	FILE *z80_cyc_trace = std::fopen("z80_cyc.trace", "w");
	FILE *m68k_trace = std::fopen("gunnailb_68k.trace", "w");
	FILE *m68k_cyc_trace = std::fopen("m68k_cyc.trace", "w");
	NmkTraceWriter trace("gunnailb_video.trace", "gunnailb", 10000000, "", "program", 0, 0xffffff, ":screen");
	Crc32 crc;
	...
	// NMI (soundlatch pending) edge tracking — gunnailb has no IM0 vector
	// arbitration to check, unlike the Seibu-sound-based ports.
	bool     prev_nmi_n = true;
	uint64_t nmi_falling_count = 0;
	...
	auto tick = [&]() {
		...
		bool nmi_n_now = top.dbg_z80_nmi_n;
		if (prev_nmi_n && !nmi_n_now) {
			nmi_falling_count++;
			if (log_irq)
				std::fprintf(stderr, "cycle=%llu z80 NMI falling edge (soundlatch write)\n", (unsigned long long)clk_sys_ticks);
		}
		prev_nmi_n = nmi_n_now;
		...
		if (oki_we_now && !oki_we_prev_dbg) {
			oki_we_count++;
			if (log_snd)
				std::fprintf(stderr, "cycle=%llu oki_we dout=%02X\n",
				             (unsigned long long)clk_sys_ticks, top.dbg_oki_chip_dout);
		}
		...
		if (!prev_frame_done && frame_done_now) {
			...
			if (std::getenv("TB_DUMP_PPM") != nullptr) {
				char fname[64];
				std::snprintf(fname, sizeof(fname), "gunnailb_frame_%02u.ppm", frame_count);
	...
	std::printf("tb_gunnailb: ran %llu clk_sys cycles (~%llu 68000 bus cycles)\n",
	            (unsigned long long)RUN_CYCLES, (unsigned long long)(RUN_CYCLES / 4));
	std::printf("tb_gunnailb: Z80 executed %llu instructions, last fetch PC=$%04X\n",
	            (unsigned long long)z80_instrs, last_z80_pc);
	std::printf("tb_gunnailb: 68000 executed %llu instructions, completed %u write bus cycles, last fetch PC=$%06X\n",
	            (unsigned long long)m68k_instrs, m68k_writes, last_fetch_pc);
	std::printf("tb_gunnailb: Z80 NMI (soundlatch write) falling edges: %llu\n", (unsigned long long)nmi_falling_count);
	std::printf("tb_gunnailb: Z80 wrote to YM2203 %u times, OKI (68000-driven) written %u times\n", ym_we_count, oki_we_count);
	std::printf("tb_gunnailb: rendered %u video frame(s), wrote gunnailb_video.trace\n", frame_count);
	...
	if (std::getenv("TB_DUMP_VRAM") != nullptr) {
		...
		std::fprintf(stderr, "tb_gunnailb: non-blank palette=%d/1024 bgvram=%d/8192 txvram=%d/2048\n",
		...
		std::fprintf(stderr, "tb_gunnailb: %d/%d rendered pixels nonzero\n", nonzero_px, SCREEN_W * SCREEN_H);
	}
	return 0;
}
