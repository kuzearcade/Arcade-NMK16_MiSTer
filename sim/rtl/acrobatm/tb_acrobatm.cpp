// Testbench for rtl/acrobatm/acrobatm_core.sv — the fifth Tier 2
// NMK004-board game to get a system-level integration testbench, after
// sim/rtl/mustang/, sim/rtl/bioship/, sim/rtl/blkheart/, and
// sim/rtl/strahl/'s own tb_*.cpp (those files' own headers cover the
// shared rationale for every trace/instrumentation technique reused
// here unchanged). acrobatm shares bioship's own exact clk_sys
// architecture (40MHz, /4=10MHz for the 68000, /5=8MHz for NMK004/pixel
// — see acrobatm_core.sv's own header), so every cycle-trace divisor
// here matches tb_bioship.cpp's own (/4 for the 68000, /5 for NMK004).
//
...
// dedicated ROM disassembly investigation has been done for acrobatm's
// own boot sequence.
#include <cstdint>
#include <cstdio>

#include "Vacrobatm_core.h"
#include "verilated.h"
...
int main(int argc, char **argv) {
	...
	Vacrobatm_core top{&contextp};
	FILE *nmk004_trace = std::fopen("nmk004_sys.trace", "w");
	FILE *nmk004_cyc_trace = std::fopen("nmk004_cyc.trace", "w");
	NmkTraceWriter trace("acrobatm_video.trace", "acrobatm", 10000000, "", "program", 0, 0xffffff, ":screen");
	Crc32 crc;
	...
	FILE *m68k_trace = std::fopen("acrobatm_68k.trace", "w");
	...
	auto tick = [&]() {
		...
		if (dbg_valid_now && !prev_dbg_valid) {
			...
			// NMK004's own real clock is clk_sys/5 here (bioship's own
			// ratio, not mustang's /4 — see acrobatm_core.sv's header).
		...
		if (!prev_frame_done && frame_done_now) {
			...
			if (std::getenv("TB_DUMP_PPM") != nullptr) {
				char fname[64];
				std::snprintf(fname, sizeof(fname), "acrobatm_frame_%02u.ppm", frame_count);
	...
	std::printf("tb_acrobatm: ran %llu clk_sys cycles (~%llu 68000 bus cycles), NMK004 executed %llu instructions (last PC=$%04X)\n",
	            (unsigned long long)RUN_CYCLES, (unsigned long long)(RUN_CYCLES / 4), (unsigned long long)nmk004_instrs, (unsigned)last_pc);
	std::printf("tb_acrobatm: 68000 last instruction-fetch PC=$%06X, completed %u write bus cycles total\n",
	            last_fetch_pc, m68k_writes);
	std::printf("tb_acrobatm: NMK004 wrote to YM2203 %u times\n", ym_we_count);
	std::printf("tb_acrobatm: NMK004 wrote to OKI1/OKI2 %u/%u times\n", oki0_we_count, oki1_we_count);
	std::printf("tb_acrobatm: rendered %u video frame(s), wrote acrobatm_video.trace\n", frame_count);
	std::printf("tb_acrobatm: 68000 executed %llu instructions, wrote acrobatm_68k.trace\n", (unsigned long long)m68k_instrs);
	...
	if (std::getenv("TB_DUMP_VRAM") != nullptr) {
		...
		std::fprintf(stderr, "tb_acrobatm: non-blank palette=%d/768 bgvram=%d/8192 txvram=%d/1024\n",
		...
		std::fprintf(stderr, "tb_acrobatm: %d/%d rendered pixels nonzero\n", nonzero_px, SCREEN_W * SCREEN_H);
	}
	return 0;
}
