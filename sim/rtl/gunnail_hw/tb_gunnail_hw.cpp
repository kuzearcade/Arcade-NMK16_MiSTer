...
int main(int argc, char **argv) {
	...
	FILE *m68k_trace = fopen("gunnail_hw_68k.trace", "w");

	FILE *f = fopen("roms/gunnail_ioctl.bin", "rb");
	if (!f) { printf("FAIL: could not open roms/gunnail_ioctl.bin\n"); return 1; }
	fseek(f, 0, SEEK_END);
	long len = ftell(f);
	fseek(f, 0, SEEK_SET);
	...
	for (; clk_sys_ticks < g_run_cycles; clk_sys_ticks++) {
		...
		if (!prev_frame_done && frame_done_now) {
			...
			if (dump_ppm) {
				char fname[64];
				std::snprintf(fname, sizeof(fname), "gunnail_hw_frame_%03u.ppm", frame_count);
			...
			if (frame_count < 5 || frame_count % 50 == 0)
				printf("tb_gunnail_hw: frame %u: %ld/%d nonzero pixels\n", frame_count, nonzero_px, SCREEN_W * SCREEN_H);
			frame_count++;
		}
...

		{
			...
			if (pv && !prev_prot_valid) { prot_instrs++; prot_last_pc = top.dbg_prot_pc; }
			prev_prot_valid = pv;
			bool pb = top.dbg_prot_bus_rd || top.dbg_prot_bus_wr;
			if (pb && !prev_prot_bus) { prot_bus_accesses++; if (log_host && prot_bus_accesses <= 40) fprintf(stderr, "F%03u PROT %s %05x\n", frame_count, top.dbg_prot_bus_wr ? "W" : "R", (unsigned)top.dbg_prot_addr); }
			prev_prot_bus = pb;
	...
	printf("tb_gunnail_hw: ran %llu clk_sys cycles\n", (unsigned long long)g_run_cycles);
	printf("tb_gunnail_hw: distinct 68000 fetch PCs in the final quarter of the run: %zu\n", recent_pcs.size());
	{
		int shown = 0;
		printf("  sample: ");
	...
	printf("tb_gunnail_hw: NMK004 executed %ld instructions, last fetch PC=$%04X\n", snd_instrs, snd_last_pc);
	printf("tb_gunnail_hw: NMK004 clock: %u pulses delivered, %u withheld for ROM fetches (%.3f%%)\n",
	...
	printf("tb_gunnail_hw: 68000 executed %ld instructions, last fetch PC=$%06X\n", m68k_instrs, m68k_last_pc);
	printf("tb_gunnail_hw: protection MCU executed %ld instructions, last PC=$%04X, %ld shared-bus accesses, 68000 HALT=%d, NMK214 config writes %u\n",
	       prot_instrs, prot_last_pc, prot_bus_accesses, (int)top.dbg_halt_68k, cfg_writes);
	printf("tb_gunnail_hw: host latch: %u commands, %u distinct replies\n", host_cmds, host_replies);
	printf("tb_gunnail_hw: NMK004 wrote to YM2203 %ld times, OKI0 %ld times, OKI1 %ld times\n", ym_writes, oki0_writes, oki1_writes);
	printf("tb_gunnail_hw: OKI ADPCM fetch audit: oki0 %u of %u sample bytes unserved at latch, oki1 %u of %u\n",
	...
	printf("tb_gunnail_hw: OKI cen stall audit: %u cen pulses, withheld by cache stall oki0 %u (%.4f%%), oki1 %u (%.4f%%)\n",
	...
	printf("tb_gunnail_hw: rendered %u video frame(s); last frame had %ld/%d nonzero pixels\n",
