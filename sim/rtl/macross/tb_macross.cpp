// Testbench for rtl/macross/macross_core.sv — the second Family D
// (TLCS-90 protection MCU) game, after tdragon1. macross shares
// macrossb's own exact clk_sys architecture (40MHz, /4=10MHz for the
// 68000, /5=8MHz for NMK004/pixel — see macross_core.sv's own header),
...
int main(int argc, char **argv) {
	...
	NmkTraceWriter trace("macross_video.trace", "macross", 10000000, "", "program", 0, 0xffffff, ":screen");
	Crc32 crc;
	...
	FILE *m68k_trace = std::fopen("macross_68k.trace", "w");
	...
	auto tick = [&]() {
		...
		if (!prev_frame_done && frame_done_now) {
			...
			if (std::getenv("TB_DUMP_PPM") != nullptr) {
				char fname[64];
				std::snprintf(fname, sizeof(fname), "macross_frame_%02u.ppm", frame_count);
	...
	std::printf("tb_macross: ran %llu clk_sys cycles (~%llu 68000 bus cycles), NMK004 executed %llu instructions (last PC=$%04X)\n",
	            (unsigned long long)RUN_CYCLES, (unsigned long long)(RUN_CYCLES / 4), (unsigned long long)nmk004_instrs, (unsigned)last_pc);
	std::printf("tb_macross: protection MCU executed %llu instructions (last PC=$%04X), 68000 HALT asserted %llu times\n",
	            (unsigned long long)prot_instrs, (unsigned)prot_last_pc, (unsigned long long)halt_asserts);
	std::printf("tb_macross: 68000 last instruction-fetch PC=$%06X, completed %u write bus cycles total\n",
	            last_fetch_pc, m68k_writes);
	std::printf("tb_macross: NMK004 wrote to YM2203 %u times\n", ym_we_count);
	std::printf("tb_macross: NMK004 wrote to OKI1/OKI2 %u/%u times\n", oki0_we_count, oki1_we_count);
	std::printf("tb_macross: rendered %u video frame(s), wrote macross_video.trace\n", frame_count);
	std::printf("tb_macross: 68000 executed %llu instructions, wrote macross_68k.trace\n", (unsigned long long)m68k_instrs);
	...
	if (std::getenv("TB_DUMP_VRAM") != nullptr) {
		...
		std::fprintf(stderr, "tb_macross: non-blank palette=%d/1024 bgvram=%d/8192 txvram=%d/1024\n",
		...
		std::fprintf(stderr, "tb_macross: %d/%d rendered pixels nonzero\n", nonzero_px, SCREEN_W * SCREEN_H);
	}
	return 0;
}
