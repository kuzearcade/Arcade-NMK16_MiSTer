...
int main(int argc, char **argv) {
	...
	// Savestate round trip (2026-09-18): TB_SS_SAVE=N saves (slot TB_SS_SLOT)
	// at frame N, TB_SS_LOAD=M loads it at frame M, and the TB_SS_CMP frames
	// rendered after each are compared pixel for pixel: after the load the
	// machine must replay exactly what it did after the save.
	...
	int ss_phase = 0;             // 0 idle, 1 save requested, 2 recording after save, 3 load requested, 4 recording after load
	int ss_rec = 0;
	std::vector<std::vector<uint32_t>> ss_after_save, ss_after_load;
	uint64_t ss_req_tick = 0;
	bool ss_pending_clear = false;
	int ss_result = -1;           // -1 not run, 0 pass, 1 fail
	...
	for (; clk_sys_ticks < g_run_cycles; clk_sys_ticks++) {
		...
		if (top.ss_done_ok || top.ss_done_fail) {
			printf("tb_hs: savestate %s %s at tick %llu (frame %u, %llu ticks after the request)%s%d\n",
				...
				if (++ss_rec >= ss_cmp) {
					if (ss_phase == 4) {
						...
						for (size_t i = 0; i < ss_after_load.size() && i < ss_after_save.size(); i++) {
							...
							if (diff) {
								if (i > 0) bad_frames++;
								printf("tb_hs: savestate frame %zu after load differs: %ld pixels%s\n", i, diff, i == 0 ? " (sprite double buffer, expected)" : "");
						...
						printf("tb_hs: SAVESTATE ROUND TRIP: %zu of %zu frames differ (frame 0 excluded) -> %s\n", bad_frames, ss_after_load.size(), bad_frames ? "FAIL" : "PASS");
					}
					ss_phase = 0;
				}
			}
			if ((long)frame_count == ss_save_frame && ss_phase == 0) { top.ss_save = 1; ss_pending_clear = true; ss_phase = 1; ss_req_tick = clk_sys_ticks; printf("tb_hs: savestate SAVE requested at frame %u\n", frame_count); }
			if ((long)frame_count == ss_load_frame && ss_phase == 0) { top.ss_load = 1; ss_pending_clear = true; ss_phase = 3; ss_req_tick = clk_sys_ticks; printf("tb_hs: savestate LOAD requested at frame %u\n", frame_count); }
