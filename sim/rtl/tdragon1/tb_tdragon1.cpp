// Testbench for rtl/tdragon1/tdragon1_core.sv — the first Family D
// (TLCS-90 protection MCU) game. Built directly on tb_tdragon.cpp (same
// 32MHz clk_sys, same /4 68000/NMK004 cycle-trace divisor — see that
// module's header for the shared rationale) with one addition: a
// second instruction-trace stream for the protection MCU itself
// (dbg_prot_pc/dbg_prot_valid, /8 divisor since it runs at 4MHz, not
// 8MHz — see rtl/tdragon1/tdragon1_core.sv's header) plus a halt-state
// transition log (dbg_halt_68k), since this is the first port in the
// project where the 68000 can genuinely be halted by another bus
// master — worth direct visibility for root-causing any divergence.
#include <cstdint>
#include <cstdio>

#include "Vtdragon1_core.h"
#include "verilated.h"
...
int main(int argc, char **argv) {
	...
	NmkTraceWriter trace("tdragon1_video.trace", "tdragon1", 8000000, "", "program", 0, 0xffffff, ":screen");
	Crc32 crc;
	...
	FILE *m68k_trace = std::fopen("tdragon1_68k.trace", "w");
	...
	auto tick = [&]() {
		...
		if (prot_dbg_valid_now && !prot_prev_dbg_valid) {
			...
			if (prot_reg_trace)
				std::fprintf(prot_reg_trace, "%llu %04X HL=%04X\n", (unsigned long long)(clk_sys_ticks / 8), pc, (unsigned)top.dbg_prot_hl);
			prot_instrs++;
			prot_last_pc = pc;
		}
		...
		// edge), matching MAME's own totalcycles sample point.
		...
		if (!prev_frame_done && frame_done_now) {
			...
			if (std::getenv("TB_DUMP_PPM") != nullptr) {
				char fname[64];
				std::snprintf(fname, sizeof(fname), "tdragon1_frame_%02u.ppm", frame_count);
	...
	std::printf("tb_tdragon1: ran %llu clk_sys cycles (~%llu 68000 bus cycles), NMK004 executed %llu instructions (last PC=$%04X)\n",
	            (unsigned long long)RUN_CYCLES, (unsigned long long)(RUN_CYCLES / 4), (unsigned long long)nmk004_instrs, (unsigned)last_pc);
	std::printf("tb_tdragon1: protection MCU executed %llu instructions (last PC=$%04X), 68000 HALT asserted %llu times\n",
	            (unsigned long long)prot_instrs, (unsigned)prot_last_pc, (unsigned long long)halt_asserts);
	std::printf("tb_tdragon1: 68000 last instruction-fetch PC=$%06X, completed %u write bus cycles total\n",
	            last_fetch_pc, m68k_writes);
	std::printf("tb_tdragon1: NMK004 wrote to YM2203 %u times\n", ym_we_count);
	std::printf("tb_tdragon1: NMK004 wrote to OKI1/OKI2 %u/%u times\n", oki0_we_count, oki1_we_count);
	std::printf("tb_tdragon1: rendered %u video frame(s), wrote tdragon1_video.trace\n", frame_count);
	std::printf("tb_tdragon1: 68000 executed %llu instructions, wrote tdragon1_68k.trace\n", (unsigned long long)m68k_instrs);
	...
	if (std::getenv("TB_DUMP_VRAM") != nullptr) {
		...
		std::fprintf(stderr, "tb_tdragon1: non-blank palette=%d/1024 bgvram=%d/8192 txvram=%d/1024\n",
		...
		std::fprintf(stderr, "tb_tdragon1: %d/%d rendered pixels nonzero\n", nonzero_px, SCREEN_W * SCREEN_H);
	}
	return 0;
}
