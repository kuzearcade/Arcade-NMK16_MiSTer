// Testbench for rtl/blkheart/blkheart_core.sv — the third Tier 2
// NMK004-board game to get a system-level integration testbench, after
// sim/rtl/mustang/tb_mustang.cpp and sim/rtl/bioship/tb_bioship.cpp
// (those files' own headers cover the shared rationale for every
// trace/instrumentation technique reused here unchanged). Unlike
// tb_bioship.cpp, blkheart shares mustang's own exact clk_sys
// architecture (32MHz, /4 for both the 68000 and NMK004/pixel clocks —
// see blkheart_core.sv's own header for why), so every cycle-trace
// divisor here is /4, not bioship's mixed /4-and-/5.
//
// Like tb_bioship.cpp, this testbench does NOT hardcode a game-specific
// host-handshake poll-loop PC or root-cause capture point — those were
// found via dedicated ROM disassembly investigations specific to
// mustang's own boot sequence, not done for blkheart.
#include <cstdint>
#include <cstdio>

#include "Vblkheart_core.h"
#include "verilated.h"
...
int main(int argc, char **argv) {
	...
	Vblkheart_core top{&contextp};
	FILE *nmk004_trace = std::fopen("nmk004_sys.trace", "w");
	FILE *nmk004_cyc_trace = std::fopen("nmk004_cyc.trace", "w");
	NmkTraceWriter trace("blkheart_video.trace", "blkheart", 8000000, "", "program", 0, 0xffffff, ":screen");
	Crc32 crc;
	...
	FILE *m68k_trace = std::fopen("blkheart_68k.trace", "w");
	...
	auto tick = [&]() {
		...
		// docs/tier2-system.md's "68000 bus-wait-state timing".
		...
		if (!prev_frame_done && frame_done_now) {
			...
			if (std::getenv("TB_DUMP_PPM") != nullptr) {
				char fname[64];
				std::snprintf(fname, sizeof(fname), "blkheart_frame_%02u.ppm", frame_count);
	...
	std::printf("tb_blkheart: ran %llu clk_sys cycles (~%llu 68000 bus cycles), NMK004 executed %llu instructions (last PC=$%04X)\n",
	            (unsigned long long)RUN_CYCLES, (unsigned long long)(RUN_CYCLES / 4), (unsigned long long)nmk004_instrs, (unsigned)last_pc);
	std::printf("tb_blkheart: 68000 last instruction-fetch PC=$%06X, completed %u write bus cycles total\n",
	            last_fetch_pc, m68k_writes);
	std::printf("tb_blkheart: NMK004 wrote to YM2203 %u times\n", ym_we_count);
	std::printf("tb_blkheart: NMK004 wrote to OKI1/OKI2 %u/%u times\n", oki0_we_count, oki1_we_count);
	std::printf("tb_blkheart: rendered %u video frame(s), wrote blkheart_video.trace\n", frame_count);
	std::printf("tb_blkheart: 68000 executed %llu instructions, wrote blkheart_68k.trace\n", (unsigned long long)m68k_instrs);
	...
	if (std::getenv("TB_DUMP_VRAM") != nullptr) {
		...
		std::fprintf(stderr, "tb_blkheart: non-blank palette=%d/1024 bgvram=%d/8192 txvram=%d/1024\n",
		...
		std::fprintf(stderr, "tb_blkheart: %d/%d rendered pixels nonzero\n", nonzero_px, SCREEN_W * SCREEN_H);
	}
	return 0;
}
