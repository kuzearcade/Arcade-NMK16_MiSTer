// Testbench for rtl/vandyke/vandyke_core.sv — the seventh Tier 2
// NMK004-board game to get a system-level integration testbench.
// vandyke shares bioship's own exact clk_sys architecture (40MHz,
// /4=10MHz for the 68000, /5=8MHz for NMK004/pixel — see
// vandyke_core.sv's own header for why), so every cycle-trace divisor
...
// dedicated ROM disassembly investigation has been done for vandyke's
// own boot sequence.
#include <cstdint>
#include <cstdio>

#include "Vvandyke_core.h"
#include "verilated.h"
...
int main(int argc, char **argv) {
	...
	Vvandyke_core top{&contextp};
	FILE *nmk004_trace = std::fopen("nmk004_sys.trace", "w");
	FILE *nmk004_cyc_trace = std::fopen("nmk004_cyc.trace", "w");
	NmkTraceWriter trace("vandyke_video.trace", "vandyke", 10000000, "", "program", 0, 0xffffff, ":screen");
	Crc32 crc;
	...
	FILE *m68k_trace = std::fopen("vandyke_68k.trace", "w");
	...
	auto tick = [&]() {
		...
		if (!prev_frame_done && frame_done_now) {
			...
			if (std::getenv("TB_DUMP_PPM") != nullptr) {
				char fname[64];
				std::snprintf(fname, sizeof(fname), "vandyke_frame_%02u.ppm", frame_count);
	...
	std::printf("tb_vandyke: ran %llu clk_sys cycles (~%llu 68000 bus cycles), NMK004 executed %llu instructions (last PC=$%04X)\n",
	            (unsigned long long)RUN_CYCLES, (unsigned long long)(RUN_CYCLES / 4), (unsigned long long)nmk004_instrs, (unsigned)last_pc);
	std::printf("tb_vandyke: 68000 last instruction-fetch PC=$%06X, completed %u write bus cycles total\n",
	            last_fetch_pc, m68k_writes);
	std::printf("tb_vandyke: NMK004 wrote to YM2203 %u times\n", ym_we_count);
	std::printf("tb_vandyke: NMK004 wrote to OKI1/OKI2 %u/%u times\n", oki0_we_count, oki1_we_count);
	std::printf("tb_vandyke: rendered %u video frame(s), wrote vandyke_video.trace\n", frame_count);
	std::printf("tb_vandyke: 68000 executed %llu instructions, wrote vandyke_68k.trace\n", (unsigned long long)m68k_instrs);
	...
	if (std::getenv("TB_DUMP_VRAM") != nullptr) {
		...
		std::fprintf(stderr, "tb_vandyke: non-blank palette=%d/1024 bgvram=%d/8192 txvram=%d/1024\n",
		...
		std::fprintf(stderr, "tb_vandyke: %d/%d rendered pixels nonzero\n", nonzero_px, SCREEN_W * SCREEN_H);
	}
	return 0;
}
