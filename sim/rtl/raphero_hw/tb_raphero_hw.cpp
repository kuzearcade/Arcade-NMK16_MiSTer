// Hardware-mode verification for raphero_core (HW_ROMS=1) — see
...
// device). Frames land in raphero_hw_frame_NN.ppm with TB_DUMP_PPM=1;
...
int main(int argc, char **argv) {
	...
	FILE *m68k_trace = fopen("raphero_hw_68k.trace", "w");

	FILE *f = fopen("roms/raphero_ioctl.bin", "rb");
	if (!f) { printf("FAIL: could not open roms/raphero_ioctl.bin\n"); return 1; }
	fseek(f, 0, SEEK_END);
	long len = ftell(f);
	fseek(f, 0, SEEK_SET);
	...
	for (; clk_sys_ticks < g_run_cycles; clk_sys_ticks++) {
		...
		if (!prev_frame_done && frame_done_now) {
			...
			if (dump_ppm) {
				char fname[64];
				std::snprintf(fname, sizeof(fname), "raphero_hw_frame_%03u.ppm", frame_count);
		...
		bool snd_valid_now = top.dbg_snd_valid;
		if (snd_valid_now && !prev_snd_valid) {
			snd_instrs++;
			snd_last_pc = top.dbg_snd_pc;
		}
	...
	printf("tb_raphero_hw: ran %llu clk_sys cycles\n", (unsigned long long)g_run_cycles);
	printf("tb_raphero_hw: distinct 68000 fetch PCs in the final quarter of the run: %zu\n", recent_pcs.size());
	{
		int shown = 0;
		printf("  sample: ");
	...
	printf("tb_raphero_hw: TLCS-90 executed %ld instructions, last fetch PC=$%04X\n", snd_instrs, snd_last_pc);
	printf("tb_raphero_hw: TLCS-90 clock: %u pulses delivered, %u withheld for ROM fetches (%.3f%%)\n",
	       (unsigned)top.dbg_snd_cen_total, (unsigned)top.dbg_snd_stall_total,
	       top.dbg_snd_cen_total ? 100.0 * top.dbg_snd_stall_total / (top.dbg_snd_cen_total + top.dbg_snd_stall_total) : 0.0);
	printf("tb_raphero_hw: 68000 executed %ld instructions, last fetch PC=$%06X\n", m68k_instrs, m68k_last_pc);
	printf("tb_raphero_hw: TLCS-90 wrote to YM2203 %ld times, OKI0 %ld times, OKI1 %ld times\n", ym_writes, oki0_writes, oki1_writes);
	printf("tb_raphero_hw: OKI ADPCM fetch audit: oki0 %u of %u sample bytes unserved at latch, oki1 %u of %u\n",
	...
	printf("tb_raphero_hw: OKI cen stall audit: %u cen pulses, withheld by cache stall oki0 %u (%.4f%%), oki1 %u (%.4f%%)\n",
