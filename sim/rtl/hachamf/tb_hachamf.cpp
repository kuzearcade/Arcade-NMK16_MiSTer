// Testbench for rtl/hachamf/hachamf_core.sv — the second Family D
// (TLCS-90 protection MCU) game, after tdragon1. hachamf shares
// hachamfb's own exact clk_sys architecture (40MHz, /4=10MHz for the
// 68000, /5=8MHz for NMK004/pixel — see hachamf_core.sv's own header),
// plus a new /10=4MHz divider for the protection MCU. Built directly on
// tb_tdragon1.cpp's own pattern: a second instruction-trace stream for
// the protection MCU and a halt-state transition log, since this is
// the second port needing direct visibility into a second CPU and a
// real HALT signal.
#include <cstdint>
#include <cstdio>

#include "Vhachamf_core.h"
#include "verilated.h"
...
int main(int argc, char **argv) {
	...
	NmkTraceWriter trace("hachamf_video.trace", "hachamf", 10000000, "", "program", 0, 0xffffff, ":screen");
	Crc32 crc;
	...
	FILE *m68k_trace = std::fopen("hachamf_68k.trace", "w");
	...
	auto tick = [&]() {
		...
		if (prot_dbg_valid_now && !prot_prev_dbg_valid) {
			...
			if (prot_reg_trace)
				std::fprintf(prot_reg_trace, "%llu %04X HL=%04X\n", (unsigned long long)(clk_sys_ticks / 10), pc, (unsigned)top.dbg_prot_hl);
			prot_instrs++;
			prot_last_pc = pc;
		}
		...
		if (!prev_frame_done && frame_done_now) {
			...
			if (std::getenv("TB_DUMP_PPM") != nullptr) {
				char fname[64];
				std::snprintf(fname, sizeof(fname), "hachamf_frame_%02u.ppm", frame_count);
	...
	std::printf("tb_hachamf: ran %llu clk_sys cycles (~%llu 68000 bus cycles), NMK004 executed %llu instructions (last PC=$%04X)\n",
	            (unsigned long long)RUN_CYCLES, (unsigned long long)(RUN_CYCLES / 4), (unsigned long long)nmk004_instrs, (unsigned)last_pc);
	std::printf("tb_hachamf: protection MCU executed %llu instructions (last PC=$%04X), 68000 HALT asserted %llu times\n",
	            (unsigned long long)prot_instrs, (unsigned)prot_last_pc, (unsigned long long)halt_asserts);
	std::printf("tb_hachamf: 68000 last instruction-fetch PC=$%06X, completed %u write bus cycles total\n",
	            last_fetch_pc, m68k_writes);
	std::printf("tb_hachamf: NMK004 wrote to YM2203 %u times\n", ym_we_count);
	std::printf("tb_hachamf: NMK004 wrote to OKI1/OKI2 %u/%u times\n", oki0_we_count, oki1_we_count);
	std::printf("tb_hachamf: rendered %u video frame(s), wrote hachamf_video.trace\n", frame_count);
	std::printf("tb_hachamf: 68000 executed %llu instructions, wrote hachamf_68k.trace\n", (unsigned long long)m68k_instrs);
	...
	if (std::getenv("TB_DUMP_VRAM") != nullptr) {
		...
		std::fprintf(stderr, "tb_hachamf: non-blank palette=%d/1024 bgvram=%d/8192 txvram=%d/1024\n",
		...
		std::fprintf(stderr, "tb_hachamf: %d/%d rendered pixels nonzero\n", nonzero_px, SCREEN_W * SCREEN_H);
	}
	return 0;
}
