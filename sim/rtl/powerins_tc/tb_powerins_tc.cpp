...
int main(int argc, char **argv) {
	...
	top.game_macross2 = 0; top.game_powerins = 1; // Power Instinct through the shared Family C core
	...
	NmkTraceWriter trace("powerins_tc_video.trace", "tdragon2", 10000000, "", "program", 0, 0xffffff, ":screen");
	Crc32 crc;
	...
	auto tick = [&]() {
		...
		if (!prev_frame_done && frame_done_now) {
			...
			if (std::getenv("TB_DUMP_PPM") != nullptr) {
				char fname[64];
				std::snprintf(fname, sizeof(fname), "powerins_tc_frame_%02u.ppm", frame_count);
	...
	std::printf("tb_powerins_tc: ran %llu clk_sys cycles (~%llu 68000 bus cycles)\n",
	            (unsigned long long)RUN_CYCLES, (unsigned long long)(RUN_CYCLES / 4));
	std::printf("tb_powerins_tc: Z80 executed %llu instructions, last fetch PC=$%04X\n",
	            (unsigned long long)z80_instrs, last_z80_pc);
	std::printf("tb_powerins_tc: 68000 executed %llu instructions, completed %u write bus cycles, last fetch PC=$%06X\n",
	            (unsigned long long)m68k_instrs, m68k_writes, last_fetch_pc);
	std::printf("tb_powerins_tc: Z80 wrote to YM2203 %u times, OKI0 %u times, OKI1 %u times\n",
	            ym_we_count, oki0_we_count, oki1_we_count);
	std::printf("tb_powerins_tc: rendered %u video frame(s), wrote powerins_tc_video.trace\n", frame_count);
	...
	if (std::getenv("TB_DUMP_VRAM") != nullptr) {
		...
		std::fprintf(stderr, "tb_powerins_tc: non-blank palette=%d/1024 bgvram=%d/32768 txvram=%d/2048\n",
		...
		std::fprintf(stderr, "tb_powerins_tc: %d/%d rendered pixels nonzero\n", nonzero_px, SCREEN_W * SCREEN_H);
	}
	return 0;
}
