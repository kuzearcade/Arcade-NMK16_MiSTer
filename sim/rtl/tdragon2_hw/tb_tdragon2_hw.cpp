...
int main(int argc, char **argv) {
	...
	for (; clk_sys_ticks < g_run_cycles; clk_sys_ticks++) {
		...
		if (top.ss_done_ok || top.ss_done_fail) {
			printf("tb_tdragon2_hw: savestate %s %s at tick %llu (frame %u, %llu ticks after the request)%s%d\n", ss_phase == 1 ? "save" : "load", top.ss_done_ok ? "OK" : "FAILED",
				...
				if (++ss_rec >= ss_cmp) {
					if (ss_phase == 4) {
						...
						for (size_t i = 0; i < ss_after_load.size() && i < ss_after_save.size(); i++) {
							...
							if (diff) {
								if (i > 0) bad_frames++;
								printf("tb_tdragon2_hw: savestate frame %zu after load differs: %ld pixels%s\n", i, diff, i == 0 ? " (sprite double buffer, expected)" : "");
						...
						printf("tb_tdragon2_hw: SAVESTATE ROUND TRIP: %zu of %zu frames differ (frame 0 excluded) -> %s\n", bad_frames, ss_after_load.size(), bad_frames ? "FAIL" : "PASS");
					}
					ss_phase = 0;
				}
			}
			if ((long)frame_count == ss_save_frame && ss_phase == 0) { top.ss_save = 1; ss_pending_clear = true; ss_phase = 1; ss_req_tick = clk_sys_ticks; printf("tb_tdragon2_hw: savestate SAVE requested at frame %u\n", frame_count); }
			if ((long)frame_count == ss_load_frame && ss_phase == 0) { top.ss_load = 1; ss_pending_clear = true; ss_phase = 3; ss_req_tick = clk_sys_ticks; printf("tb_tdragon2_hw: savestate LOAD requested at frame %u\n", frame_count); }
			frame_count++;
		}
	...
	if (ss_save_frame >= 0) printf("tb_tdragon2_hw: savestate result: %s\n", ss_result == 0 ? "PASS" : ss_result == 1 ? "FAIL" : "INCOMPLETE (run longer)");
