...
int main(int argc, char **argv) {
	...
	top.game_macross2 = (game_byte >> 0) & 1; top.game_powerins = (game_byte >> 1) & 1;
	top.game_tdragon3h = (game_byte >> 2) & 1; top.game_pi_bootleg = (game_byte >> 3) & 1;
	top.game_pi_nosnd = (game_byte >> 4) & 1; top.game_pi_gfxlsb = (game_byte >> 5) & 1;
	const char *prefix = std::getenv("TB_PREFIX") ? std::getenv("TB_PREFIX") : "fc_mg";
	...
	NmkTraceWriter trace("fc_mg_video.trace", "tdragon2", 10000000, "", "program", 0, 0xffffff, ":screen");
	Crc32 crc;
	...
	auto tick = [&]() {
		...
		if (!prev_frame_done && frame_done_now) {
			...
			if (std::getenv("TB_DUMP_PPM") != nullptr) {
				char fname[64];
				std::snprintf(fname, sizeof(fname), "%s_frame_%02u.ppm", prefix, frame_count);
	...
	std::printf("tb_fc_mg: ran %llu clk_sys cycles (~%llu 68000 bus cycles)\n",
	            (unsigned long long)RUN_CYCLES, (unsigned long long)(RUN_CYCLES / 4));
	std::printf("tb_fc_mg: Z80 executed %llu instructions, last fetch PC=$%04X\n",
	            (unsigned long long)z80_instrs, last_z80_pc);
	std::printf("tb_fc_mg: 68000 executed %llu instructions, completed %u write bus cycles, last fetch PC=$%06X\n",
	            (unsigned long long)m68k_instrs, m68k_writes, last_fetch_pc);
	std::printf("tb_fc_mg: Z80 wrote to YM2203 %u times, OKI0 %u times, OKI1 %u times\n",
	            ym_we_count, oki0_we_count, oki1_we_count);
	std::printf("tb_fc_mg: rendered %u video frame(s), wrote fc_mg_video.trace\n", frame_count);
	...
	if (std::getenv("TB_DUMP_VRAM") != nullptr) {
		...
		std::fprintf(stderr, "tb_fc_mg: non-blank palette=%d/1024 bgvram=%d/32768 txvram=%d/2048\n",
		...
		std::fprintf(stderr, "tb_fc_mg: %d/%d rendered pixels nonzero\n", nonzero_px, SCREEN_W * SCREEN_H);
	}
	return 0;
}
