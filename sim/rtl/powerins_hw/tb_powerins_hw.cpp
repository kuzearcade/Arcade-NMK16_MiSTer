...
int main(int argc, char **argv) {
	...
	FILE *m68k_trace = fopen("powerins_hw_68k.trace", "w");

	FILE *f = fopen("roms/powerins_ioctl.bin", "rb");
	if (!f) { printf("FAIL: could not open roms/powerins_ioctl.bin\n"); return 1; }
	fseek(f, 0, SEEK_END);
	long len = ftell(f);
	fseek(f, 0, SEEK_SET);
	...
	for (; clk_sys_ticks < g_run_cycles; clk_sys_ticks++) {
		...
		if (!prev_frame_done && frame_done_now) {
			...
			if (dump_ppm && (frame_count % ppm_step) == 0) {
				char fname[64];
				std::snprintf(fname, sizeof(fname), "powerins_hw_frame_%02u.ppm", frame_count);
			...
			if (frame_count < 5 || frame_count % 50 == 0)
				printf("tb_powerins_hw: frame %u: %ld/%d nonzero pixels\n", frame_count, nonzero_px, SCREEN_W * SCREEN_H);
			if (autoplay) {
	...
	printf("tb_powerins_hw: ran %llu clk_sys cycles\n", (unsigned long long)g_run_cycles);
	printf("tb_powerins_hw: sprite audit: pass clks %u, stall clks %u, units %u, late swaps %u, longest pass %u clks\n",
	...
	printf("tb_powerins_hw: CPU audit: bus cycles %u, ROM cycles %u, ROM misses %u, rom_wait clks %u, ram_wait clks %u, dma_wait clks %u, miss latency avg %.2f max %u clks\n",
	...
	printf("tb_powerins_hw: distinct 68000 fetch PCs in the final quarter of the run: %zu\n", recent_pcs.size());
	{
		int shown = 0;
		printf("  sample: ");
	...
	printf("tb_powerins_hw: Z80 executed %ld instructions, last fetch PC=$%04X\n", z80_instrs, z80_last_pc);
	printf("tb_powerins_hw: 68000 executed %ld instructions, last fetch PC=$%06X\n", m68k_instrs, m68k_last_pc);
	printf("tb_powerins_hw: Z80 wrote to YM2203 %ld times, OKI0 %ld times, OKI1 %ld times\n", ym_writes, oki0_writes, oki1_writes);
	printf("tb_powerins_hw: OKI ADPCM fetch audit: oki0 %u of %u sample bytes unserved at latch, oki1 %u of %u\n",
	...
	printf("tb_powerins_hw: OKI cen stall audit: %u cen pulses, withheld by cache stall oki0 %u (%.4f%%), oki1 %u (%.4f%%)\n",
	...
	printf("tb_powerins_hw: final dbg_z80_reset_n=%d dbg_z80_m1_n=%d dbg_z80_mreq_n=%d\n",
	       top.dbg_z80_reset_n, top.dbg_z80_m1_n, top.dbg_z80_mreq_n);
	printf("tb_powerins_hw: rom_csum=%04X count=%u done=%d (reference value for real-hardware comparison)\n",
	       (unsigned)top.rom_csum_o, (unsigned)top.rom_csum_count_o, (int)top.rom_csum_done_o);
	printf("tb_powerins_hw: rom_fetch_csum=%04X count=%u done=%d (reference value for real-hardware comparison)\n",
	       (unsigned)top.rom_fetch_csum_o, (unsigned)top.rom_fetch_csum_count_o, (int)top.rom_fetch_csum_done_o);
	printf("tb_powerins_hw: ioctl_csum=%04X count=%u done=%d (reference value for real-hardware comparison)\n",
	...
	printf("tb_powerins_hw: rom_fetch_bucket dump (bucket state touched):\n");
	for (int b = 0; b < 128; b++) {
	...
	printf("tb_powerins_hw: rom_fetch_fine dump (fine state touched):\n");
	for (int b = 0; b < 128; b++) {
		top.dbg_bucket_sel_i = b;
		top.eval();
	...
	printf("tb_powerins_hw: rom_fetch_word dump (word state touched):\n");
	for (int b = 0; b < 16; b++) {
		top.dbg_bucket_sel_i = b;
		top.eval();
		printf("WORD %3d %04X %d\n", b, (unsigned)top.dbg_word_state_o, (int)top.dbg_word_touched_o);
	}
	printf("tb_powerins_hw: rendered %u video frame(s); last frame had %ld/%d nonzero pixels\n",
