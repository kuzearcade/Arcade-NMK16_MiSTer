#!/usr/bin/env python3
"""Generate the Family C ("NMK16_Macross2" rbf) clone .mra files.
...
# NMK16_Macross2.sv's <switches> byte 2 bits 2-5). Each has its own SDRAM
...
def mra(setname, parent, desc, game_line, maincpu, overrides, dip_overrides=None):
    ...
    out = []
    out.append(f"""<!--
  {desc} — clone of {parent} on the shared Family C "NMK16_Macross2" rbf.
...
<misterromdescription>
  ...
  <rbf>NMK16_Macross2</rbf>
""")
    if p["rotation"]:
        out.append("\n" + ROTATION_NOTE + f"  <rotation>{ROTATION_TEXT}</rotation>\n")
    out.append(f"""
